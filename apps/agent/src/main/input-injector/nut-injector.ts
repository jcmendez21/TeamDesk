...
const MOUSE_BUTTON = (button: 0 | 1 | 2): unknown => {
  const n = loadNut();
 ...
 */
function mapKey(code: string): unknown {
  const n = loadNut();
  // Letters: KeyA..KeyZ
  if (/^Key[A-Z]$/.test(code)) return n.Key[code.slice(3)];
  // Digits: Digit0..Digit9
  if (/^Digit[0-9]$/.test(code)) return n.Key[`Num${code.slice(5)}`] ?? n.Key.Num0;
  // Function keys: F1..F12
  if (/^F([1-9]|1[0-2])$/.test(code)) return n.Key[code];
...
export class NutInjector implements InputInjector {
  async inject(msg: InputMsg): Promise<void> {
    ...
    switch (msg.type) {
      ...
      case 'wheel': {
        ...
        if (ticks > 0) await n.mouse.scrollDown(ticks);
        else if (ticks < 0) await n.mouse.scrollUp(-ticks);
        return;
      }
      case 'keydown': {
      ...
      case 'type-text': {
        ...
        if (msg.text) await n.keyboard.type(msg.text);
        return;
      }
      case 'cad': {
